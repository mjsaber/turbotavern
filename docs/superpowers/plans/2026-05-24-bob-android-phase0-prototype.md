...
**Tech Stack:**
- **Go 1.25+** (driven by current `golang.org/x/mobile` requiring Go 1.25); mihomo pinned to `v1.19.25` (recorded in `android/bobcore/PINNED-VERSIONS.md` after Spike A)
- Kotlin 2.0+，AGP 8.7+（Phase 0 skeleton 没用 Compose；Phase 1 切 Compose）
...
- [ ] **能进 BG 战斗**: 手动一次确认账户状态
- [ ] **gomobile** —— pinned in `android/bobcore/go.mod` via `tool` directive; `build-aar.sh` materializes pinned gobind every run. No `@latest` install required.
...
```bash
cd android/bobcore
./build-aar.sh   # wraps `go tool gomobile bind` with NDK auto-detect + gobind version verification
# Effective invocation (recorded in PINNED-VERSIONS.md):
