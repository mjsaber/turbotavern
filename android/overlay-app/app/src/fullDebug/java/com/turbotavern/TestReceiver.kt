...
import com.turbotavern.core.CloseResult
import com.turbotavern.core.DebugConnectionCoreOverride
import com.turbotavern.core.DebugForegroundOverride
import com.turbotavern.core.MihomoCore
import java.io.File
...
class TestReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (cmd) {
            ...
            "devrec_mark" -> {
                val s = com.turbotavern.devrec.DevRecorderService.live
                if (s == null) Log.i(TAG, "devrec_mark: not recording")
                else { context.startService(Intent(context, s::class.java)
                    .setAction(com.turbotavern.devrec.DevRecorderService.ACTION_MARK)); Log.i(TAG, "devrec_mark dispatched") }
            }
            "devrec_stop" -> {
                val s = com.turbotavern.devrec.DevRecorderService.live
                if (s == null) Log.i(TAG, "devrec_stop: not recording")
                else { context.startService(Intent(context, s::class.java)
                    .setAction(com.turbotavern.devrec.DevRecorderService.ACTION_STOP)); Log.i(TAG, "devrec_stop dispatched") }
            }
            "sim_set_snapshot" -> {
    ...
    private fun stateLabel(s: com.turbotavern.overlay.OverlayState): String =
        when (s) {
            com.turbotavern.overlay.OverlayState.WaitingForBattle -> "Waiting"
            com.turbotavern.overlay.OverlayState.Ready -> "Ready"
            com.turbotavern.overlay.OverlayState.Cooldown -> "Cooldown"
        }
    ...
    companion object {
        ...
        const val ACTION = "com.turbotavern.TEST"
